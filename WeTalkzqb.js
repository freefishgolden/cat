...
const DEBUG = true; // 发布时可改为 false 减少日志输出
...
function runAccount(acc, index, total) {
  ...
  console.log(`   UA Seed: ${acc.uaSeed}`);
  if (DEBUG) console.log(`   User-Agent: ${ua.substring(0, 80)}...`);
  console.log(`${'='.repeat(40)}`);

  // 封装请求，自动打印请求/响应日志
  function fetchApi(path, label = path) {
    const url = buildUrl(path, acc.capture);
    if (DEBUG) {
      console.log(`📡 [${label}] 请求 URL: ${url}`);
      console.log(`   Headers: ${JSON.stringify(headers, null, 2).substring(0, 300)}...`);
    } else {
      console.log(`📡 [${label}] 请求中...`);
    }
    ...
    return $task.fetch({ url, method: 'GET', headers })
      .then(res => {
        ...
        console.log(`✅ [${label}] 响应 ${res.statusCode} (${duration}ms)`);
        if (DEBUG && res.body && res.body.length < 500) {
          console.log(`   Body: ${res.body}`);
        } else if (DEBUG) {
          console.log(`   Body 长度: ${res.body ? res.body.length : 0} 字符`);
        }
        return res;
      })
      .catch(err => {
        console.log(`❌ [${label}] 请求失败: ${err.error || JSON.stringify(err)}`);
        throw err;
      });
  }
  ...
  function doVideoLoop(count) {
    let i = 0;
    console.log(`🎬 开始领取 ${count} 次视频奖励`);

    function next() {
      if (i >= count) {
        console.log(`🏁 视频奖励全部完成`);
        return Promise.resolve();
      }

      return new Promise(resolve => {
        const delay = i === 0 ? 1500 : VIDEO_DELAY;
        if (DEBUG) console.log(`⏳ 等待 ${delay}ms 后请求第 ${i+1}/${count} 次视频...`);

        setTimeout(() => {
          i++;
          console.log(`🎥 请求 videoBonus (第 ${i}/${count} 次)`);
          fetchApi('videoBonus', `视频奖励${i}`)
            .then(res => {
              try {
                ...
                if (d.retcode === 0) {
                  ...
                  console.log(`💰 视频${i} 获得: ${bonus} Coins`);
                  resolve(next());
                } else {
                  msgs.push(`⏸ 视频${i}：${d.retmsg}`);
                  console.log(`⚠️ 视频${i} 返回: ${d.retmsg}`);
                  resolve();
                }
              } catch (e) {
                msgs.push(`❌ 视频${i}：解析失败`);
                console.log(`🔥 视频${i} JSON 解析错误: ${e}`);
                resolve();
              }
            })
            .catch(err => {
              msgs.push(`❌ 视频${i}：${err.error || '请求失败'}`);
              console.log(`🔥 视频${i} 异常: ${err.error || String(err)}`);
              resolve();
            });
        }, delay);
      });
  ...
  // 执行流程
  return fetchApi('queryBalanceAndBonus', '查询余额')
    .then(res => {
      try {
        const d = JSON.parse(res.body);
        if (d.retcode === 0) {
          msgs.push(`💰 余额：${d.result.balance} Coins`);
          console.log(`💰 初始余额: ${d.result.balance}`);
        } else {
          msgs.push(`⚠️ 查询：${d.retmsg}`);
          console.log(`⚠️ 查询返回: ${d.retmsg}`);
        }
      } catch (e) {
        msgs.push('❌ 查询：解析失败');
        console.log(`🔥 查询 JSON 解析错误: ${e}`);
      }
      return fetchApi('checkIn', '签到');
    })
    .then(res => {
      try {
        const d = JSON.parse(res.body);
        if (d.retcode === 0) {
          ...
          msgs.push(`✅ 签到：${hint}`);
          console.log(`✅ 签到结果: ${hint}`);
        } else {
          msgs.push(`⚠️ 签到：${d.retmsg}`);
          console.log(`⚠️ 签到返回: ${d.retmsg}`);
        }
      } catch (e) {
        msgs.push('❌ 签到：解析失败');
        console.log(`🔥 签到 JSON 解析错误: ${e}`);
      }
      return doVideoLoop(MAX_VIDEO);
    })
    .then(() => fetchApi('queryBalanceAndBonus', '最终余额'))
    .then(res => {
      try {
        const d = JSON.parse(res.body);
        if (d.retcode === 0) {
          msgs.push(`💰 最新余额：${d.result.balance} Coins`);
          console.log(`💰 最终余额: ${d.result.balance}`);
        }
      } catch (e) {
        console.log(`🔥 最终余额解析失败: ${e}`);
      }
      const totalTime = Date.now() - stepStart;
      console.log(`🟢 ${tag} 处理完成，总耗时 ${totalTime}ms`);
      console.log(`${'='.repeat(40)}\n`);
      return msgs.join('\n');
    })
    .catch(err => {
      msgs.push(`❌ 异常：${err.error || String(err)}`);
      console.log(`🔥 ${tag} 发生致命错误: ${err.error || String(err)}`);
      if (DEBUG) console.log(err.stack || '无堆栈');
      console.log(`${'='.repeat(40)}\n`);
      return msgs.join('\n');
    });
}
